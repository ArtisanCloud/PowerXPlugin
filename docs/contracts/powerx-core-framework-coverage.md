...
| Agent | 18（6 lifecycle + 12 Session） | `runtime/powerx/agent`、`runtime/agent` | partial | .NET 已有 Lifecycle 与 Session UUID DTO、显式 `ILocalAgentLifecycleStore` / `ILocalAgentStore`、启动期 local/delegated 选择点；Session 的 N402 delegated 12 项 transport 与 SSE `Last-Event-ID` 恢复已实现并通过定向测试。Lifecycle 当前只有 local service 和 delegated 注入接口，尚无 .NET typed delegated lifecycle client；18 项整体仍不能标记 ready_for_integration。插件的生产 local store 和安装态 grant/跨租户验收也需分别证明，不得用 local fallback。 |
