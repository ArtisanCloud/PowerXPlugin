...
- 后端会自动读取 `skeleton/backend/.env`；请在该文件中设置 `POWERX_PROVIDER_MODE/POWERX_PROXY/PLUGIN_IAM_*` 等变量。
- Standalone 本地数据源应保持 `POWERX_PROVIDER_MODE=local`；是否接宿主链路只由 `POWERX_PROXY` 表示。
