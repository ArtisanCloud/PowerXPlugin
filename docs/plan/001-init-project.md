**PowerXPlugin 仓库的目标**是同时承载可运行的 Skeleton 以及可复用的“框架层”能力（供 PowerXPluginNote 等外部插件引用），并通过 `github.com/ArtisanCloud/PowerXPlugin/framework/backend/go/...`（Go/Gin 默认实现）与 `@artisan-cloud/plugin-framework-admin` / `@artisan-cloud/plugin-framework-client` 输出统一组件生态。
...
- 将 skeleton 中的公共装配、系统端点、Manifest/RBAC 逻辑沉淀到 `framework/backend/go`，通过根模块 `github.com/ArtisanCloud/PowerXPlugin/framework/backend/go/...` 暴露为默认 Go 实现。
...
- [ ] 外部插件通过 import `github.com/ArtisanCloud/PowerXPlugin/framework/backend/go` 编译通过。
...
│  ├─ go.mod                                 # module github.com/ArtisanCloud/PowerXPlugin/framework/backend/go（默认 Go 实现的 import path）
...
* **其他插件项目**（如 PowerXPluginNote）只**import 框架**：
  ...
  * 后端：`github.com/ArtisanCloud/PowerXPlugin/framework/backend/go/...`（当前默认 Go 实现）
...
├─ plugin.yaml
├─ backend/
│  ├─ go.mod                         # require github.com/ArtisanCloud/PowerXPlugin/framework/backend/go vX.Y.Z
...
import (
  ...
  "github.com/ArtisanCloud/PowerXPlugin/framework/backend/go/rbac"
...
> 位置：`framework/backend/go/bootstrap`（对外 import 为 `github.com/ArtisanCloud/PowerXPlugin/framework/backend/go/bootstrap`）
...
* Go 实现目录（`framework/backend/go/middleware`，对外 import: `github.com/ArtisanCloud/PowerXPlugin/framework/backend/go/middleware`）：`Recovery()`, `Trace()`, `Audit()`, `AuthGuard()`
* Go 实现目录（`framework/backend/go/observability`，import: `github.com/ArtisanCloud/PowerXPlugin/framework/backend/go/observability`）：`InitMetrics(app)`, `InitTracing(app)`
* Go 实现目录（`framework/backend/go/tenancy`，import: `github.com/ArtisanCloud/PowerXPlugin/framework/backend/go/tenancy`）：`WithTenant(ctx, tenantID string) context.Context`, `TenantFrom(ctx)`
