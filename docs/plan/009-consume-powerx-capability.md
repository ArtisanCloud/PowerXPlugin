...
| 3. 调用封装 | 后端推荐使用 Go `powerx/integration` SDK（`integration.NewClient().Invoke(ctx, capabilityId, payload)`），并通过插件后端暴露受控 API 给前端复用；保持能力 ID 常量化，便于限流配置。 |
...
1. **配置面板**：提供 Capability ID 下拉（读取 manifest `requiredCapabilities`）、Action 输入框、JSON Payload 编辑器，并允许选择 `X-PowerX-Tenant`、切换 `PX_USE_MOCK`。选择 REST Action 时会提示必须填写 `preferred_protocol + method + endpoint`，并可一键插入模板（含 query/headers/body 字段）；gRPC Action 则提示 service/method。
