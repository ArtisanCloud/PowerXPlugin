...
```
/_p/com.powerx.plugins.base/api/*  →  http://powerx-plugin-base:8078
```
...
| `POWERX_DEV_MODE`        | `0`     | 开发环境语义（1 表示开发环境） |
| `POWERX_AUTH_OPTIONAL`   | `false` | 是否临时放宽鉴权（仅本地调试） |
...
| `curl -v localhost:8078/v1/ping`            | 验证服务可用性   |
