...
func TestReadFailureAndCancellationContract(t *testing.T) {
	for _, scenario := range []string{"read_failure", "read_cancel", "token_cancel", "token_failure", "empty_token"} {
		t.Run(scenario, func(t *testing.T) {
			...
			c := &Client{baseURL: "http://test.invalid", http: &http.Client{Transport: transport}, token: TokenProviderFunc(token)}
			err := func() error { var out map[string]any; return c.do(ctx, "GET", "/test", nil, &out) }()
			if scenario == "read_cancel" || scenario == "token_cancel" {
				...
				var typed *HTTPError
				if !errors.As(err, &typed) || typed.StatusCode != expected || typed.ReasonCode != "CAPABILITY_UPSTREAM_DEPENDENCY" {
