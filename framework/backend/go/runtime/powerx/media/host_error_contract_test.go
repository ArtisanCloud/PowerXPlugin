	...
	for _, status := range []int{400, 401, 403, 404, 409, 422, 429, 502, 503} {
		for _, shape := range []string{"reason_code", "error_code"} {
			t.Run(fmt.Sprintf("%d/%s", status, shape), func(t *testing.T) {
				...
				defer server.Close()
				client, err := NewHostClientWithTokenProvider(HostClientConfig{BaseURL: server.URL}, HostTokenProviderFunc(func(context.Context) (string, error) { return "sts", nil }), server.Client())
				if err != nil {
					t.Fatal(err)
				}
				_, err = client.GetAsset(context.Background(), "00000000-0000-4000-8000-000000000001")
				var e *HostHTTPError
