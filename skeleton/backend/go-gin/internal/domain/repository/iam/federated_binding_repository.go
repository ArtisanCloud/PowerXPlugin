...
type FederatedBindingRepository struct {
	*EntityRepo.BaseRepository[iammodel.FederatedBinding]
}

func NewFederatedBindingRepository(db *gorm.DB) *FederatedBindingRepository {
	return &FederatedBindingRepository{BaseRepository: EntityRepo.NewBaseRepository[iammodel.FederatedBinding](db)}
}

func (r *FederatedBindingRepository) List(ctx context.Context, tenantUUID, provider string) ([]iammodel.FederatedBinding, error) {
	...
	rows := make([]iammodel.FederatedBinding, 0)
...
func (r *FederatedBindingRepository) Save(ctx context.Context, binding *iammodel.FederatedBinding) (*iammodel.FederatedBinding, error) {
	if binding == nil {
...
func (r *FederatedBindingRepository) Unbind(ctx context.Context, tenantUUID, provider, externalUserID string) (*iammodel.FederatedBinding, error) {
