...
// CustomerAccount mirrors the PowerX Core customer_accounts table for local dev.
// Production customer master data must be stored in PowerX Core.
type CustomerAccount struct {
	models.BaseNoTenantModel

	CustomerUUID  string            `gorm:"column:customer_uuid;type:uuid;not null;uniqueIndex" json:"customer_uuid"`
...
type CustomerAuthIdentity struct {
	...
	Provider        string            `gorm:"column:provider;type:varchar(32);not null;uniqueIndex:uk_customer_identity_subject,priority:1;index" json:"provider"`
...
func (c *CustomerAuthIdentity) TableName() string {
	return models.S(models.TableCustomerAuthIdentities)
}

type CustomerTenantMembership struct {
	models.BaseNoTenantModel

	MembershipUUID string            `gorm:"column:membership_uuid;type:uuid;not null;uniqueIndex" json:"membership_uuid"`
	TenantUUID     string            `gorm:"column:tenant_uuid;type:uuid;not null;uniqueIndex:uk_customer_tenant_membership,priority:1;index" json:"tenant_uuid"`
	...
	Source         string            `gorm:"column:source;type:varchar(32);not null;default:'local_dev';index" json:"source"`
	ExpiresAt      *time.Time        `gorm:"column:expires_at;index" json:"expires_at,omitempty"`
	Metadata       datatypes.JSONMap `gorm:"column:metadata;type:jsonb;default:'{}'::jsonb" json:"metadata,omitempty"`
}

func (c *CustomerTenantMembership) TableName() string {
	return models.S(models.TableCustomerTenantMemberships)
}

type MiniAppEntry struct {
	models.BaseNoTenantModel

	EntryUUID  string            `gorm:"column:entry_uuid;type:uuid;not null;uniqueIndex" json:"entry_uuid"`
	...
	Theme      datatypes.JSONMap `gorm:"column:theme;type:jsonb;default:'{}'::jsonb" json:"theme,omitempty"`
	Features   datatypes.JSONMap `gorm:"column:features;type:jsonb;default:'{}'::jsonb" json:"features,omitempty"`
	Status     string            `gorm:"column:status;type:varchar(32);not null;default:'active';index" json:"status"`
	ExpiresAt  *time.Time        `gorm:"column:expires_at;index" json:"expires_at,omitempty"`
	Metadata   datatypes.JSONMap `gorm:"column:metadata;type:jsonb;default:'{}'::jsonb" json:"metadata,omitempty"`
}

func (m *MiniAppEntry) TableName() string {
	return models.S(models.TableMiniAppEntries)
}

type CustomerSession struct {
	models.BaseNoTenantModel

	SessionUUID      string            `gorm:"column:session_uuid;type:uuid;not null;uniqueIndex" json:"session_uuid"`
	CustomerUUID     string            `gorm:"column:customer_uuid;type:uuid;not null;index" json:"customer_uuid"`
	TenantUUID       string            `gorm:"column:tenant_uuid;type:uuid;index" json:"tenant_uuid,omitempty"`
...
func (c *CustomerSession) TableName() string {
	return models.S(models.TableCustomerSessions)
}
...
func (c *CustomerLoginEvent) TableName() string {
	return models.S(models.TableCustomerLoginEvents)
}
