...
func NewSchedulerEventDispatcher(cfg *config.Config, emitter fweventbridge.Emitter, logger *logrus.Entry) *SchedulerEventDispatcher {
	...
	if logger == nil {
		logger = logrus.WithField("component", "integration.scheduler_dispatcher")
	}
	...
	if err != nil {
		if d.logger != nil {
			d.logger.WithError(err).WithFields(logrus.Fields{
				...
				"trace_id": traceID,
				"job_name": jobName,
			}).Warn("failed to emit scheduler trigger event")
		}
		return "", err
	}
	return traceID, nil
