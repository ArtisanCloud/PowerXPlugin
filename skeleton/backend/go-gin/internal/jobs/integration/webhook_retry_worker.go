...
func NewWebhookRetryWorker(
	...
	if logger == nil {
		logger = pxlog.WithComponent("integration.webhook_retry_worker")
	}
...
func (w *WebhookRetryWorker) Run(ctx context.Context) error {
	if w.service == nil || w.attempts == nil || w.subscriptions == nil {
		pxlog.WarnCtx(pxlog.WithLogFields(ctx, map[string]interface{}{
			"module":     "integration",
			"biz_scene":  "webhook_retry_run",
			"biz_domain": "integration",
			"component":  "integration.webhook_retry_worker",
		}), "webhook retry worker dependencies not configured; skipping run")
		return nil
	}
	...
	for _, attempt := range attempts {
		...
		if moveToDLQ {
			if err := w.service.UpdateAttemptStatus(ctx, attempt.ID, model.AttemptStatusDLQ, attempt.RetryCount, nil, attempt.LastError, sub.TenantUuid); err != nil {
				pxlog.WarnCtx(pxlog.WithLogFields(ctx, map[string]interface{}{
					"module":     "integration",
					"biz_scene":  "webhook_retry_mark_dlq",
					"biz_domain": "integration",
					"component":  "integration.webhook_retry_worker",
					"attempt_id": attempt.ID,
					"error":      err.Error(),
				}), "failed to mark attempt as DLQ")
				continue
			}
		...
		if err != nil {
			pxlog.WarnCtx(pxlog.WithLogFields(ctx, map[string]interface{}{
				"module":     "integration",
				"biz_scene":  "webhook_retry_schedule",
				"biz_domain": "integration",
				"component":  "integration.webhook_retry_worker",
				"attempt_id": attempt.ID,
				"error":      err.Error(),
