...
func (s *BindingService) List(ctx context.Context, tenantUUID, provider string) ([]identitymodel.FederatedBinding, error) {
...
func (s *BindingService) Bind(ctx context.Context, in BindInput) (*identitymodel.FederatedBinding, error) {
	...
	binding := &identitymodel.FederatedBinding{
...
func (s *BindingService) Unbind(ctx context.Context, tenantUUID, provider, externalUserID string) (*identitymodel.FederatedBinding, error) {
	return s.UnbindScoped(ctx, tenantUUID, provider, "", externalUserID)
}

func (s *BindingService) UnbindScoped(ctx context.Context, tenantUUID, provider, tenantScope, externalUserID string) (*identitymodel.FederatedBinding, error) {
	if strings.TrimSpace(tenantUUID) == "" {
		return nil, ErrTenantUUIDRequired
	}
...
func (s *BindingService) memberInTenant(ctx context.Context, memberID uint64, tenantUUID string) bool {
	...
	var count int64
	_ = s.db.WithContext(ctx).Model(&identitymodel.Member{}).Where("id = ? AND tenant_uuid = ?", memberID, tenantUUID).Count(&count).Error
	return count > 0
}
