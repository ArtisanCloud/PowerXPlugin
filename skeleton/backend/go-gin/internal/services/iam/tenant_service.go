...
func (s *TenantService) ResolveByIdentifier(ctx context.Context, identifier string) (*iamm.Tenant, error) {
	if s == nil || s.db == nil {
		return nil, errors.New("iam: tenant service unavailable")
	}
	search := strings.TrimSpace(identifier)
	if search == "" {
		return nil, errors.New("tenant identifier required")
	}
	var tenant iamm.Tenant
	base := s.db.WithContext(ctx).Model(&iamm.Tenant{}).Where("status = ?", iamm.StatusActive)
	if err := base.Where("lower(key) = ? OR lower(uuid) = ?", strings.ToLower(search), strings.ToLower(search)).First(&tenant).Error; err == nil {
		return &tenant, nil
	...
	if id, err := strconv.ParseUint(search, 10, 64); err == nil && id > 0 {
		if err := s.db.WithContext(ctx).
			Model(&iamm.Tenant{}).
			Where("id = ? AND status = ?", id, iamm.StatusActive).
	...
	if err := s.db.WithContext(ctx).
		Model(&iamm.Tenant{}).
		Where("status = ? AND lower(name) LIKE ?", iamm.StatusActive, like).
...
func (s *TenantService) ListWithFilter(ctx context.Context, filter TenantListFilter) (*TenantListResult, error) {
	...
	memberCount, err := s.loadTenantMemberCount(ctx, tenants)
	if err != nil {
		return nil, err
	}
	return &TenantListResult{Items: tenants, Total: total, MemberCount: memberCount}, nil
}

func (s *TenantService) loadTenantMemberCount(ctx context.Context, tenants []iamm.Tenant) (map[string]int64, error) {
	result := map[string]int64{}
	if s == nil || s.db == nil || len(tenants) == 0 {
		return result, nil
	}
	uuids := make([]string, 0, len(tenants))
	...
	if err := s.db.WithContext(ctx).
		Model(&iamm.Member{}).
		Select("tenant_uuid, COUNT(*) AS total").
		Where("tenant_uuid IN ?", uuids).
		Group("tenant_uuid").
		Scan(&rows).Error; err != nil {
		return nil, err
	}
	for _, row := range rows {
		uuid := strings.TrimSpace(strings.ToLower(row.TenantUUID))
		if uuid == "" {
			continue
		}
		result[uuid] = row.Total
	}
	return result, nil
}
