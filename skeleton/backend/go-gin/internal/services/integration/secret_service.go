...
func NewSecretService(
	...
	log := pxlog.WithComponent("integration.secret_service")
	var approvalSvc *ApprovalService
	if approvalRepo != nil {
		approvalSvc = NewApprovalService(approvalRepo, pxlog.WithComponent("integration.secret_service.approval"))
	}
	if provider == nil {
		provider = NewRandomSecretProvider(pxlog.WithComponent("integration.secret_service.provider"))
	}
...
func (s *SecretService) CompleteRotation(ctx context.Context, tenantID, secretID, actor string) (*model.SecretCredential, error) {
	...
	if err := s.provider.Revoke(ctx, tenantID, secret.CurrentSecretRef); err != nil {
		pxlog.WarnCtx(pxlog.WithLogFields(ctx, map[string]interface{}{
			"module":     "integration",
			"biz_scene":  "secret_complete_rotation",
			"biz_domain": "integration",
			"component":  "integration.secret_service",
			"secret_id":  secret.ID,
			"error":      err.Error(),
		}), "failed to revoke previous secret_ref")
	}
...
func (s *SecretService) RevokeSecret(ctx context.Context, tenantID, secretID, actor string) (*model.SecretCredential, error) {
	...
	if secret.CurrentSecretRef != "" {
		if err := s.provider.Revoke(ctx, tenantID, secret.CurrentSecretRef); err != nil {
			pxlog.WarnCtx(pxlog.WithLogFields(ctx, map[string]interface{}{
				"module":     "integration",
				"biz_scene":  "secret_revoke",
				"biz_domain": "integration",
				"component":  "integration.secret_service",
				"secret_id":  secret.ID,
				"error":      err.Error(),
			}), "failed to revoke current secret")
		}
	}
	if secret.PendingSecretRef != "" {
		if err := s.provider.Revoke(ctx, tenantID, secret.PendingSecretRef); err != nil {
			pxlog.WarnCtx(pxlog.WithLogFields(ctx, map[string]interface{}{
				...
				"secret_id":  secret.ID,
				"error":      err.Error(),
			}), "failed to revoke pending secret")
		}
	}
...
func (s *SecretService) submitApproval(ctx context.Context, tenantID, targetID, action string, payload any, actor string) {
	...
	if _, err := s.approvals.SubmitChange(ctx, req); err != nil {
		pxlog.WarnCtx(pxlog.WithLogFields(ctx, map[string]interface{}{
			"module":     "integration",
			"biz_scene":  "secret_submit_approval",
			"biz_domain": "integration",
			"component":  "integration.secret_service",
			"secret_id":  targetID,
			"error":      err.Error(),
		}), "failed to submit approval entry")
	}
}
