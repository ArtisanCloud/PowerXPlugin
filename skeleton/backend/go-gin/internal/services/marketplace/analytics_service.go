...
func NewAnalyticsService(
	...
	if logger == nil {
		logger = pxlog.WithComponent("marketplace_analytics_service")
	}
			...
			if listing != nil && strings.TrimSpace(listing.VendorID) != "" && s.notificationRepo != nil {
				...
				if err := s.notificationRepo.QueueNotification(ctx, &payload); err != nil {
					pxlog.WarnCtx(pxlog.WithLogFields(ctx, map[string]interface{}{
						...
						"license_id":  license.ID,
						"metric":      dayAfter.Metric,
						"biz_scene":   "usage_alert_queue",
						"biz_domain":  "marketplace",
						"component":   "marketplace_analytics_service",
						"error":       err.Error(),
					}), "failed to queue usage alert notification")
				}
			}
	...
	if listing != nil && strings.TrimSpace(listing.VendorID) != "" && totalGross > 0 {
		...
		if report, err := s.EnsureRevenueReport(ctx, tenantID, listing.VendorID, periodStart, periodEnd, totalGross, currency); err != nil {
			pxlog.WarnCtx(pxlog.WithLogFields(ctx, map[string]interface{}{
				...
				"biz_scene":   "revenue_report_upsert",
				"biz_domain":  "marketplace",
				"component":   "marketplace_analytics_service",
				"error":       err.Error(),
			}), "failed to upsert revenue report")
		} else {
