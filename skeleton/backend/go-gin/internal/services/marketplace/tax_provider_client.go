...
func NewTaxProviderClient(cfg *config.Config, httpClient *http.Client, logger *pxlog.Entry) (*TaxProviderClient, error) {
	if cfg == nil {
	...
	for i := 0; i < attempts; i++ {
		...
		if c.logger != nil {
			pxlog.WarnCtx(pxlog.WithLogFields(ctx, map[string]interface{}{
				...
				"provider":   c.provider,
				"attempt":    i + 1,
				"biz_scene":  "tax_provider_dispatch",
				"biz_domain": "marketplace",
				"component":  "marketplace.tax_provider_client",
				"error":      err.Error(),
			}), "tax provider dispatch failed, retrying")
		}
	}
	...
	for i := 0; i < attempts; i++ {
		...
		if c.logger != nil {
			pxlog.WarnCtx(pxlog.WithLogFields(ctx, map[string]interface{}{
				...
				"provider":   c.provider,
				"attempt":    i + 1,
				"biz_scene":  "tax_provider_replay",
				"biz_domain": "marketplace",
				"component":  "marketplace.tax_provider_client",
				"error":      err.Error(),
			}), "tax provider replay failed, retrying")
		}
	}
...
func newStripeAdapter(httpClient *http.Client, cfg config.IntegrationStripeTaxConfig, logger *pxlog.Entry) providerAdapter {
...
func newAvalaraAdapter(httpClient *http.Client, cfg config.IntegrationAvalaraConfig, logger *pxlog.Entry) providerAdapter {
