...
func (h *Handler) options(c *gin.Context) (metadatasvc.ListOptions, bool) {
	if h.isDelegated() {
		if h.delegated == nil {
			admincommon.ProviderUnavailable(c, "METADATA_PROVIDER_NOT_CONFIGURED", "metadata delegated provider is not configured", h.diagnostics())
	...
	if h.local == nil {
		admincommon.ProviderUnavailable(c, "METADATA_PROVIDER_NOT_CONFIGURED", "local metadata store is unavailable", h.diagnostics())
		return metadatasvc.ListOptions{}, false
	}
	return metadatasvc.ListOptions{
...
func (h *Handler) diagnostics() admincommon.ProviderDiagnostics {
	...
	if h != nil && strings.TrimSpace(h.mode.String()) != "" {
		mode = h.mode.String()
	}
	return admincommon.NewProviderDiagnostics(mode, h != nil && h.delegated != nil, h != nil && h.local != nil)
}
