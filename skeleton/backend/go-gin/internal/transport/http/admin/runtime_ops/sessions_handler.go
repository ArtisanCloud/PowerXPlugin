...
func (h *SessionsHandler) Register(c *gin.Context) {
	...
	if !h.publishEvent(stored.TenantUuid, stored.ID, "session.registered", stored) {
		contracts.ResponseServiceUnavailable(c, "mcp realtime publish is unavailable", nil)
		return
	}
...
func (h *SessionsHandler) Ack(c *gin.Context) {
	...
	if !h.publishEvent(updated.TenantUuid, updated.ID, "session.ready", updated) {
		contracts.ResponseServiceUnavailable(c, "mcp realtime publish is unavailable", nil)
		return
	}
...
func (h *SessionsHandler) Heartbeat(c *gin.Context) {
	...
	if !h.publishEvent(updated.TenantUuid, updated.ID, "session.heartbeat", map[string]any{
...
func (h *SessionsHandler) Close(c *gin.Context) {
	...
	if !h.publishEvent(updated.TenantUuid, updated.ID, "session.closed", map[string]any{
		"reason": req.Reason,
	}) {
		contracts.ResponseServiceUnavailable(c, "mcp realtime publish is unavailable", nil)
		return
...
func (h *SessionsHandler) Invoke(c *gin.Context) {
	...
	if !h.publishEvent(req.TenantUuid, sessionID, "invoke.completed", map[string]any{
		"session_id":     sessionID,
...
func (h *SessionsHandler) publishEvent(tenantUUID, sessionID, eventType string, payload interface{}) bool {
	if h == nil || h.broker == nil || sessionID == "" {
		return false
	}
