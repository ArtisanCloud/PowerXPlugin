...
func RequireCapabilityGateway(c *gin.Context, deps *app.Deps) bool {
	...
	if deps.CapabilityGateway == nil || !deps.CapabilityGateway.Enabled() {
		...
		contracts.ResponseErrorWithDetails(c, 503, "SERVICE_UNAVAILABLE", "capability gateway unavailable", gin.H{
			...
			"iam_mode": detectGatewayIAMMode(deps),
		})
		return false
	}
	return true
...
func shouldEnforceDelegatedContract(deps *app.Deps) bool {
	...
	return strings.EqualFold(detectGatewayIAMMode(deps), "delegated")
}

func detectGatewayIAMMode(deps *app.Deps) string {
	if deps == nil {
		return "unknown"
	}
	if deps.IAMMode == iamservice.IAMModeDelegated {
	...
	mode := strings.ToLower(strings.TrimSpace(deps.Config.Context.IAMMode))
	if mode == "" {
		return "unknown"
	}
	return mode
