...
func (h *FederatedHandler) WeComServerCallback(c *gin.Context) {
	...
	if tenantUUID == "" || corpID == "" || appID == "" {
		logrus.WithFields(logrus.Fields{
			...
			"corp_id":     corpID,
			"app_id":      appID,
		}).Warn("wecom callback path params missing")
	...
	if err != nil {
		logrus.WithFields(logrus.Fields{
			...
			"app_id":      appID,
			"error":       err.Error(),
		}).Warn("wecom callback config resolve failed")
	...
	}).Info("wecom callback url resolved")
	...
	if err != nil {
		logrus.WithFields(logrus.Fields{
			...
			"x_forwarded_proto": strings.TrimSpace(c.GetHeader("X-Forwarded-Proto")),
			"error":             err.Error(),
		}).Warn("wecom callback sdk init failed")
	...
	if c.Request.Method == http.MethodGet {
		...
		if err != nil {
			...
			}).Warn("wecom callback verify url failed")
	...
	if err != nil {
		logrus.WithFields(logrus.Fields{
			...
			"error":          err.Error(),
			"callback_route": c.Request.URL.Path,
		}).Warn("wecom callback notify failed")
