...
// baseRepository 模拟 constitution 中的 BaseRepository 能力。
type baseRepository[T any] struct{}

// WithTenantTx 在 skeleton 中模拟 BeginTenantTx/WithTenantTx 的语义：
// 将租户 ID 写入上下文后执行回调。
...
// TemplateRepository 提供内存版仓储实现。
type TemplateRepository struct {
	*baseRepository[Template]

	mu      sync.RWMutex
	store   map[uint64]map[uint64]*Template // tenantID -> templateID -> Template
	seq     map[uint64]uint64               // tenantID -> auto increment sequence
