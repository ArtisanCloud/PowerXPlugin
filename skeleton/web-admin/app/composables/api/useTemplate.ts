...
export function useTemplateApi() {
  const api = usePluginApi({ pluginId: "com.powerx.sample" });

  const listTemplates = () => api.get<ListResponse>("/templates");
  const getTemplate = (id: number | string) => api.get(`/templates/${id}`);
  const createTemplate = (payload: Partial<Template>) =>
    api.post("/templates", payload);
  const updateTemplate = (id: number | string, payload: Partial<Template>) =>
