    ...
    <header class="space-y-2">
      ...
      <div class="space-y-1">
        ...
        <p class="text-gray-600 dark:text-gray-300 max-w-3xl">
          模拟租户选择价格计划并完成 License
          购买，验证续费与离线延展能力。用于演示 Billing/License Server
          的闭环体验。
        </p>
      </div>
    </header>
          ...
          <div v-if="selectedListing">
            ...
            >
              ...
              <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                类型：{{ selectedPlan.plan_type }} · 币种：{{
                  selectedPlan.currency
                }}
                <span v-if="selectedPlan.amount != null">
                  · 金额：{{ selectedPlan.amount }}</span
                >
              </p>
              <ul
                v-if="selectedPlan.tiers?.length"
                class="mt-3 space-y-1 text-sm text-gray-600 dark:text-gray-300"
              >
                <li v-for="tier in selectedPlan.tiers" :key="tier.id">
                  {{ tier.metric }}: {{ tier.range_from }} -
                  {{ tier.range_to ?? "∞" }} → {{ tier.unit_amount }}/{{
                    tier.unit_name || "unit"
                  }}
                </li>
              </ul>
      ...
      <UCard>
        ...
        <div class="space-y-4">
          ...
          <div v-else class="space-y-4">
            <div
              class="rounded border border-gray-200 p-4 dark:border-gray-700 space-y-2"
            >
              <div class="flex items-center justify-between">
                ...
                <UBadge :color="licenseBadgeColor" class="uppercase">{{
                  license.status
                }}</UBadge>
              </div>
              <p class="text-sm text-gray-500 dark:text-gray-400">
                计划 {{ license.plan_id }} · 过期时间
                {{ formatIso(license.expires_at) }}
              </p>
              <p
                v-if="license.offline_until"
...
const tenantUuid = ref("tenant-demo");
...
const selectedPlan = computed<MarketplacePricingPlan | null>(() => {
  ...
  return (
    selectedListing.value.pricing_plans?.find(
...
function ensureTenantQuery(): Record<string, string> {
  const id = tenantUuid.value.trim();
  return id ? { tenant_uuid: id } : {};
}
