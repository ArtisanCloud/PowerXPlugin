    ...
    <header class="space-y-2">
      ...
      <div class="space-y-1">
        ...
        <p class="text-gray-600 dark:text-gray-300 max-w-3xl">
          Track active ToolGrants, review revocation history, and inspect usage
          events to ensure least-privilege enforcement across agents.
        </p>
      </div>
    </header>
      ...
      <template #default>
        <div class="grid gap-6 lg:grid-cols-2">
          <UCard class="border border-gray-200/70 dark:border-gray-700/80">
            ...
            <div
              v-if="!loading && !revocations.length"
          ...
          <UCard class="border border-gray-200/70 dark:border-gray-700/80">
            ...
            <div
              v-if="!loading && !usageEvents.length"
              class="py-6 text-center text-gray-500 dark:text-gray-400"
            >
      ...
      <UCard>
        ...
        <template #footer>
          <div class="flex justify-end">
            <UButton color="primary" @click="metadataOpen = false"
              >Close</UButton
            >
          </div>
        </template>
...
const loadData = async () => {
  if (!tenantUuid.value) {
    toast.add({ title: "Tenant UUID required", color: "red" });
    return;
  }
  loading.value = true;
  try {
    const [revResp, usageResp] = await Promise.all([
      $fetch<{ data: ToolGrantRevocation[] }>(
        `${runtimeConfig.public.apiBaseUrl}/admin/security/toolgrants/revocations`,
        {
          params: { tenant_uuid: tenantUuid.value },
          credentials: "include",
        }
      ),
      $fetch<{ data: ToolGrantUsageEvent[] }>(
        `${runtimeConfig.public.apiBaseUrl}/admin/security/toolgrants/usage`,
        {
          params: { tenant_uuid: tenantUuid.value },
          credentials: "include",
        }
...
const showMetadata = (row: ToolGrantUsageEvent) => {
  ...
  metadataJson.value = row.metadata
