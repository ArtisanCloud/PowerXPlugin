        ...
        >
          <template #capability_id-data="{ row }">
            <div class="flex flex-col">
              <span class="font-semibold text-gray-900 dark:text-white">{{ row.capability_id }}</span>
          ...
          <template #status-data="{ row }">
            <div class="flex flex-col gap-1">
              <UBadge
                :label="lifecycleBadge(row.status).label"
                :color="lifecycleBadge(row.status).color"
                variant="soft"
              />
              <span v-if="row.updatedAt" class="text-xs text-gray-500">
                {{ $t("capabilities.lifecycle.catalog.updatedAt", { time: row.updatedAt }) }}
              </span>
            </div>
          </template>
          <template #actions-data="{ row }">
            <UButton
              ...
              @click="openManager(row.capability_id)"
            >
        ...
        <div class="space-y-6">
          <div class="flex flex-col gap-2">
            <div class="flex items-start justify-between gap-4">
              ...
              <UButton icon="i-heroicons-x-mark" variant="ghost" color="neutral" @click="closeManager" />
            </div>
