<template>
  <aside
    class="h-full min-h-0 w-64 min-w-64 max-w-64 flex-shrink-0 overflow-y-auto overscroll-contain border-r border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900"
  >
    <nav class="p-4 space-y-6">
      ...
      <div v-if="isRoot">
        <div class="px-3 py-2 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
          {{ t('navigation.powerxBaseCapabilities') }}
        </div>
        <UButton
          to="/powerx/host-contract-lab"
          variant="ghost"
          color="neutral"
          class="w-full justify-start"
          :class="{
            'bg-primary-50 text-primary-600 dark:bg-primary-950 dark:text-primary-400':
              isExactActive('/powerx/host-contract-lab'),
          }"
        >
          <UIcon name="i-heroicons-squares-2x2" class="w-4 h-4 mr-3" />
          {{ t('navigation.hostContractLab') }}
        </UButton>
        <UButton
