<template>
  <UContainer class="py-8 space-y-6">
    ...
    <UModal
      ...
      :ui="{ content: 'max-h-[calc(100vh-2rem)] max-w-3xl w-full overflow-y-auto' }"
    >
      <template #content>
        <div class="space-y-5 p-6">
          ...
          <div class="flex flex-wrap items-center justify-end gap-2 border-t border-gray-200 pt-4 dark:border-gray-800">
            <UButton color="primary" variant="soft" icon="i-heroicons-user-group" @click="openContacts">
              {{ t("customerContact.manage") }}
            </UButton>
            <UButton v-if="selectedAccount" color="primary" variant="soft" icon="i-heroicons-pencil-square" :disabled="isReadOnly" @click="openEdit(selectedAccount)">
              {{ t("customerContact.editCustomerProfile") }}
            </UButton>
          </div>
        </div>
      </template>
    ...
    <UModal
      v-model:open="showContactsModal"
      :title="t('customerContact.title')"
      :description="t('customerContact.description')"
      :ui="{ content: 'max-w-3xl w-full' }"
    >
      <template #content>
        <div class="space-y-5 p-6">
          <div class="flex items-center justify-between gap-3">
            <div>
              <h2 class="text-lg font-semibold text-gray-900 dark:text-white">{{ t("customerContact.title") }}</h2>
              <p class="mt-1 text-sm text-gray-600 dark:text-gray-300">{{ t("customerContact.description") }}</p>
            </div>
            <div class="flex items-center gap-2">
              <UButton icon="i-heroicons-arrow-path" color="neutral" variant="soft" :loading="contactsLoading" @click="loadContacts">{{ t("customerContact.refresh") }}</UButton>
              <UButton icon="i-heroicons-plus" color="primary" :disabled="isReadOnly" @click="openContactCreate">{{ t("customerContact.create") }}</UButton>
            </div>
          </div>

          <UAlert v-if="contactsError" color="error" variant="soft" :title="contactsError" />
          <template v-else>
            <div class="flex flex-wrap items-center gap-2">
              <UInput v-model="contactSearch" icon="i-heroicons-magnifying-glass" :placeholder="t('customerContact.search')" class="min-w-56 flex-1" @keyup.enter="resetContactsAndLoad" />
              <USelect v-model="contactStatus" :items="contactFilterStatusOptions" class="w-36" @update:model-value="resetContactsAndLoad" />
            </div>
            <UTable :columns="contactColumns" :data="contacts.items" :loading="contactsLoading">
              <template #status-cell="{ row }"><UBadge :color="statusColor(row.original.status)" variant="soft">{{ row.original.status }}</UBadge></template>
              <template #roles-cell="{ row }"><span class="text-sm text-gray-600 dark:text-gray-300">{{ row.original.roles?.join(', ') || "-" }}</span></template>
              <template #tags-cell="{ row }"><span class="text-sm text-gray-600 dark:text-gray-300">{{ row.original.tags?.join(', ') || t("customerContact.noTags") }}</span></template>
              <template #updated-cell="{ row }"><span class="text-sm text-gray-600 dark:text-gray-300">{{ formatDate(row.original.updated_at || row.original.created_at) }}</span></template>
              <template #actions-cell="{ row }"><UButton size="xs" variant="soft" color="neutral" icon="i-heroicons-pencil-square" :disabled="isReadOnly" @click="openContactEdit(row.original)">{{ t("common.edit") }}</UButton></template>
            </UTable>
            <div v-if="!contactsLoading && contacts.items.length === 0" class="rounded-lg border border-dashed border-gray-300 p-5 text-sm text-gray-500 dark:border-gray-700 dark:text-gray-400">{{ t("customerContact.empty") }}</div>
            <div class="flex items-center justify-between border-t border-gray-200 pt-3 text-sm text-gray-500 dark:border-gray-800 dark:text-gray-400">
              <span>{{ t("customerContact.total", { total: contacts.total }) }}</span>
              <UPagination v-if="contacts.total > contactPageSize" v-model:page="contactPage" :total="contacts.total" :items-per-page="contactPageSize" show-edges />
            </div>
          </template>
        </div>
      </template>
    ...
    <UModal v-model:open="showContactFormModal" :title="contactFormMode === 'create' ? t('customerContact.create') : t('customerContact.edit')" :ui="{ content: 'max-w-2xl w-full' }">
      <template #content>
        <form class="space-y-4 p-6" @submit.prevent="submitContact">
          <div class="grid gap-4 md:grid-cols-2">
            <UFormField :label="t('customerContact.displayName')" required class="md:col-span-2"><UInput v-model="contactForm.display_name" :placeholder="t('customerContact.displayNamePlaceholder')" /></UFormField>
            <UFormField :label="t('customerContact.givenName')"><UInput v-model="contactForm.given_name" /></UFormField>
            <UFormField :label="t('customerContact.familyName')"><UInput v-model="contactForm.family_name" /></UFormField>
            <UFormField :label="t('customerContact.status')"><USelect v-model="contactForm.status" :items="contactStatusOptions" class="w-full" /></UFormField>
            <UFormField :label="t('customerContact.tags')">
              <UInput v-model="contactForm.tags" :placeholder="t('customerContact.tagsPlaceholder')" />
              <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ t("customerContact.tagsHelp") }}</p>
            </UFormField>
          </div>
          <p v-if="contactCreateError" class="text-sm text-error-600 dark:text-error-400">{{ contactCreateError }}</p>
          <div class="flex justify-end gap-2"><UButton type="button" color="neutral" variant="ghost" :disabled="creatingContact" @click="showContactFormModal = false">{{ t("common.cancel") }}</UButton><UButton type="submit" color="primary" :loading="creatingContact">{{ contactFormMode === "create" ? t("customerContact.create") : t("common.save") }}</UButton></div>
        </form>
      </template>
    </UModal>
  </UContainer>
...
const contactStatusOptions = [
  { label: t("customerContact.statusActive"), value: "active" },
  { label: t("customerContact.statusInactive"), value: "inactive" },
  { label: t("customerContact.statusTemporary"), value: "temporary" },
];
const contactFilterStatusOptions = [
  { label: t("customerContact.statusAll"), value: "all" },
...
const contactColumns = [
  { accessorKey: "display_name", header: t("customerContact.displayName") },
  { accessorKey: "status", header: t("customerContact.status"), id: "status" },
  { accessorKey: "roles", header: t("customerContact.roles") },
  { accessorKey: "tags", header: t("customerContact.tags"), id: "tags" },
  { accessorKey: "updated_at", header: t("customerContact.updatedAt"), id: "updated" },
  { accessorKey: "actions", header: t("customerContact.actions"), id: "actions" },
];
...
async function loadContacts() {
  const customerUUID = selectedAccount.value?.customer_uuid;
  if (!customerUUID) return;
  contactsLoading.value = true;
  contactsError.value = "";
  try {
    contacts.value = (await api.listContacts(customerUUID, {
      page: contactPage.value,
      page_size: contactPageSize,
      q: contactSearch.value.trim() || undefined,
      status: contactStatus.value === "all" ? undefined : contactStatus.value,
    })).data;
  } catch (error: any) {
    contactsError.value = contactErrorMessage(error, "loadFailed");
  } finally {
    contactsLoading.value = false;
  }
}

function resetContactsAndLoad() {
  contactPage.value = 1;
  loadContacts();
}

function resetContactForm() {
  Object.assign(contactForm, { display_name: "", given_name: "", family_name: "", status: "active", tags: "" });
  contactCreateError.value = "";
}
...
function openContactCreate() {
  ...
  editingContactUUID.value = "";
  resetContactForm();
  showContactFormModal.value = true;
}

function openContactEdit(contact: CustomerContact) {
  if (isReadOnly.value) return;
  contactFormMode.value = "edit";
  editingContactUUID.value = contact.contact_uuid;
...
async function submitContact() {
  if (isReadOnly.value || !selectedAccount.value?.customer_uuid) return;
  if (!contactForm.display_name.trim()) { contactCreateError.value = t("customerContact.displayNameRequired"); return; }
  const tags = parseContactTags();
  if (tags === null) {
    contactCreateError.value = t("customerContact.invalidTags");
    return;
  }
  ...
  try {
    const input = {
      ...
      given_name: contactForm.given_name.trim() || undefined,
      family_name: contactForm.family_name.trim() || undefined,
      status: contactForm.status,
      tags,
    };
    if (contactFormMode.value === "create") {
      await api.createContact(selectedAccount.value.customer_uuid, {
        ...input,
        creation_intent: contactForm.status === "temporary" ? "explicit_temporary" : "explicit_create",
      });
    } else {
      await api.updateContact(selectedAccount.value.customer_uuid, editingContactUUID.value, input);
    }
    showContactFormModal.value = false;
    resetContactForm();
    await loadContacts();
  } catch (error: any) {
    contactCreateError.value = contactErrorMessage(error, "createFailed");
  } finally {
    creatingContact.value = false;
  }
}

function parseContactTags(): string[] | null {
  const tags = contactForm.tags
    .split(/[,，;；\s]+/)
    .map((value) => value.trim())
    .filter(Boolean);
  if (tags.length > 20 || tags.some((tag) => !/^[a-z0-9][a-z0-9._-]{0,63}$/.test(tag))) return null;
  if (tags.reduce((total, tag) => total + tag.length, 0) > 1024) return null;
  return tags;
}

function contactErrorMessage(error: any, fallbackKey: "loadFailed" | "createFailed") {
  const code = String(
  ...
  if (code === "CONTACT_LOCAL_UNAVAILABLE") return t("customerContact.localUnavailable");
  if (code === "CONTACT_DELEGATE_UNAVAILABLE") return t("customerContact.delegateUnavailable");
  if (code === "CONTACT_CUSTOMER_MEMBERSHIP_INACTIVE") return t("customerContact.customerUnavailable");
  if (code === "CONTACT_INVALID_ARGUMENT") return t("customerContact.invalidInput");
  return t(`customerContact.${fallbackKey}`);
}
...
watch(contactPage, () => {
  if (showContactsModal.value) loadContacts();
});
watch(tenantSearch, () => {
