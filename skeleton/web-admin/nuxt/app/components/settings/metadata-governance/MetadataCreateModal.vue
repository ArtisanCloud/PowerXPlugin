<template>
  <UModal
    ...
    :title="t(`metadataGovernance.create.${target}.title`)"
    :description="t(`metadataGovernance.create.${target}.description`)"
  ...
  >
    <template #body>
      <UAlert
        ...
        :title="t('metadataGovernance.create.errorTitle')"
        :description="error"
      />
      ...
      <form class="space-y-4" @submit.prevent="submit">
        ...
        <div v-if="needsDefinitionFields" class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <UFormField :label="t('metadataGovernance.form.module')" required>
            <USelectMenu
              v-model="form.module"
              value-key="value"
              label-key="label"
              ...
              :ui="selectMenuUi"
              class="w-full"
              :placeholder="t('metadataGovernance.form.modulePlaceholder')"
              :search-input="{ placeholder: t('metadataGovernance.form.moduleSearchPlaceholder') }"
            />
          </UFormField>
          <UFormField :label="t('metadataGovernance.form.namespaceSuffix')" required>
            <div class="flex w-full overflow-hidden rounded-md ring ring-inset ring-accented focus-within:ring-2 focus-within:ring-primary">
              <div class="flex max-w-[45%] shrink-0 items-center truncate bg-muted px-3 text-sm text-muted" :title="namespacePrefix">
                {{ namespacePrefix }}
              </div>
              <UInput
        ...
        <div v-if="needsResourceTypeField" class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <UFormField :label="t('metadataGovernance.form.resourceType')" required>
            <UInput v-model="form.resourceType" class="w-full" :placeholder="t('metadataGovernance.form.resourceTypePlaceholder')" />
          </UFormField>
          <UFormField :label="t('metadataGovernance.form.module')" required>
            <USelectMenu
              v-model="form.module"
        ...
        <div v-if="target === 'tag'" class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <UFormField :label="t('metadataGovernance.form.resourceType')" required>
            <USelectMenu
              ...
              :ui="selectMenuUi"
              class="w-full"
              :placeholder="t('metadataGovernance.form.resourceTypeSelectPlaceholder')"
              :search-input="{ placeholder: t('metadataGovernance.form.resourceTypeSearchPlaceholder') }"
            />
          </UFormField>
          <UFormField :label="t('metadataGovernance.form.namespaceSuffix')" required>
            <div class="flex w-full overflow-hidden rounded-md ring ring-inset ring-accented focus-within:ring-2 focus-within:ring-primary">
        ...
        <div v-if="hasCodeField" class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <UFormField :label="t('metadataGovernance.form.machineIdentifier')" required>
            <UInput v-model="form.code" class="w-full" :placeholder="t('metadataGovernance.form.machineIdentifierPlaceholder')" />
          </UFormField>
          <UFormField v-if="target === 'tag'" :label="t('metadataGovernance.form.color')">
            <UInput v-model="form.color" class="w-full" type="color" />
          </UFormField>
        </div>

        <div v-if="target === 'dictionaryItem'" class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <UFormField :label="t('metadataGovernance.form.sortOrder')">
        ...
        <div v-if="target === 'taxonomyNode'" class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <UFormField :label="t('metadataGovernance.form.parentNode')">
        ...
        <div v-if="target === 'taxonomy'" class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <UFormField :label="t('metadataGovernance.form.maxDepth')" required>
        ...
        <div v-if="target === 'resourceType'" class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <UFormField :label="t('metadataGovernance.form.validatorKey')">
            <UInput v-model="form.validatorKey" class="w-full" :placeholder="t('metadataGovernance.form.validatorKeyPlaceholder')" />
          </UFormField>
          <UFormField :label="t('metadataGovernance.form.bindingEnabled')">
        ...
        <div class="rounded-lg border border-gray-200 p-4 dark:border-gray-800">
          <div class="mb-3 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div>
              <div class="text-sm font-medium text-gray-900 dark:text-white">
                {{ t("metadataGovernance.form.requiredLocaleSection") }}
              </div>
              <div class="text-xs text-gray-500">
                {{ t("metadataGovernance.form.requiredLocaleDescription") }}
              </div>
            </div>
          ...
          <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
            <UFormField :label="t('metadataGovernance.form.localizedName')" required>
              <UInput v-model="form.names[requiredLocale]" class="w-full" :placeholder="t('metadataGovernance.form.localizedNamePlaceholder')" />
            </UFormField>
            <UFormField :label="t('metadataGovernance.form.localizedDescription')">
              <UTextarea
                v-model="form.descriptions[requiredLocale]"
                class="w-full"
                :rows="3"
                :placeholder="t('metadataGovernance.form.localizedDescriptionPlaceholder')"
              />
            </UFormField>
          </div>
        </div>
        ...
        <div class="rounded-lg border border-gray-200 p-4 dark:border-gray-800">
          <div class="mb-3">
            <div class="text-sm font-medium text-gray-900 dark:text-white">
              {{ t("metadataGovernance.form.optionalLocaleSection") }}
            </div>
            <div class="text-xs text-gray-500">
              {{ t("metadataGovernance.form.optionalLocaleDescription") }}
            </div>
          </div>
          ...
          <div class="mb-4 max-w-md">
            <USelectMenu
              ...
              class="w-full"
              :placeholder="t('metadataGovernance.form.localeSelectPlaceholder')"
              :search-input="{ placeholder: t('metadataGovernance.form.localeSearchPlaceholder') }"
            />
          </div>

          <div v-if="selectedOptionalLocale" class="grid grid-cols-1 gap-4 md:grid-cols-2">
            <UFormField :label="t('metadataGovernance.form.localizedName')" :required="requiresEnglishName && selectedOptionalLocale === englishLocale">
              <UInput v-model="form.names[selectedOptionalLocale]" class="w-full" :placeholder="t('metadataGovernance.form.localizedNamePlaceholder')" />
            </UFormField>
            <UFormField :label="t('metadataGovernance.form.localizedDescription')">
              <UTextarea
          ...
          <div v-if="filledOptionalLocales.length > 0" class="mt-3 flex flex-wrap gap-2">
            <UBadge v-for="localeItem in filledOptionalLocales" :key="localeItem.value" color="neutral" variant="subtle">
              {{ localeItem.label }}
            </UBadge>
          </div>
        </div>
...
type MetadataCreateTarget = "dictionaryNamespace" | "dictionaryItem" | "taxonomy" | "taxonomyNode" | "tag" | "resourceType";
...
const localeOptions = computed(() => {
  ...
  return Array.from(values).map((value) => ({
    value,
    label: t(`metadataGovernance.form.locale.${value.replace("-", "_")}`),
  }));
});
const requiredLocaleLabel = computed(() => t(`metadataGovernance.form.locale.${requiredLocale.replace("-", "_")}`));
...
const selectedResourceType = computed(() => resourceTypeOptions.value.find((item) => item.value === form.resourceType));
...
const namespaceSuffixPlaceholder = computed(() =>
  props.target === "tag"
    ? t("metadataGovernance.form.namespaceSuffixPlaceholderTag")
    : t("metadataGovernance.form.namespaceSuffixPlaceholder"),
);
const parentNodeItems = computed(() => [
  { label: t("metadataGovernance.form.rootNode"), value: ROOT_NODE },
  ...(props.taxonomyNodes ?? []).map((node) => ({
    label: `${"  ".repeat(Math.max(0, Number(node.depth || 1) - 1))}${node.display_name || node.code || ""}`,
    value: node.uuid,
  })),
]);
...
const submit = () => {
  ...
  if (!String(form.names[requiredLocale] || "").trim()) {
    validationError.value = t("metadataGovernance.create.validation.localizedNameRequired");
    return;
  }
...
const failRequired = () => {
  validationError.value = t("metadataGovernance.create.validation.required");
};
const failNoResourceType = () => {
  validationError.value = t("metadataGovernance.create.validation.resourceTypeRequired");
};
const failEnglishNameRequired = () => {
  validationError.value = t("metadataGovernance.create.validation.englishNameRequired");
};
const failMachineIdentifierInvalid = () => {
  validationError.value = t("metadataGovernance.create.validation.machineIdentifierInvalid");
};
const failNamespaceModuleMismatch = () => {
  validationError.value = t("metadataGovernance.create.validation.namespaceModuleMismatch");
};
...
const isMachineIdentifier = (value: string) => MACHINE_IDENTIFIER_RE.test(String(value || "").trim());
