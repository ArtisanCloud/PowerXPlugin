...
const permissions = computed(() => {
  ...
  return (listData.value.items || []).filter((permission) => {
    ...
    return [permission.resource, permission.action, permission.description, permission.meta?.label]
      .filter(Boolean)
...
<template>
  <section class="space-y-4">
    <div class="flex items-center justify-between gap-3">
      <div>
        <h2 class="text-xl font-semibold text-gray-800 dark:text-white">{{ $t("organization.permission.title") }}</h2>
        <p class="text-sm text-gray-500 dark:text-gray-300">{{ $t("organization.permission.description") }}</p>
      </div>
    ...
    <div v-else class="divide-y rounded-lg border border-gray-200 dark:border-slate-700">
      <div v-for="permission in permissions" :key="permission.permission_uuid" class="p-4">
        <div class="font-medium text-gray-900 dark:text-white">{{ permission.meta?.label || [permission.resource, permission.action].join(':') }}</div>
