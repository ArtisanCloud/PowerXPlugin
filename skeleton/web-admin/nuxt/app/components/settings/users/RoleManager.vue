...
const cloneSourceUUID = ref<string | null>(null);
...
const openCloneForm = (role: Role) => {
  ...
  roleForm.scope = (role.scope || role.scope_type || "tenant") as
    ...
    | "system";
  roleForm.tenant_uuid = role.tenant_uuid || selectedTenantUuid.value;
  cloneSourceUUID.value = role.role_uuid;
  showForm.value = true;
};
...
const saveRole = async () => {
  ...
  try {
    if (isEditing.value && editingRoleUUID.value !== null) {
      ...
      await roleStore.updateRole(editingRoleUUID.value, updateData);
      ...
      const createData: RoleCreateParams = {
        ...
        description: roleForm.description,
        scope_type: roleForm.scope,
		clone_role_uuid: cloneSourceUUID.value ?? undefined,
...
const loadRoleDetail = async (role: Role) => {
  const detail = await roleStore.fetchRole(role.role_uuid);
...
const openPermissionDrawer = async (role: Role) => {
  ...
  try {
    ...
    const detail = await loadRoleDetail(role);
		permissionSelection.value = [...(detail?.permission_uuids ?? [])];
  } catch (err: any) {
    toast.add({
      title: t("common.error"),
...
const savePermissionSelection = async () => {
  ...
  try {
		await iamService.replaceRolePermissions(selectedRoleDetail.value.role_uuid, {
			tenant_uuid: tenantUuid,
			permission_uuids: permissionSelection.value,
		});
...
const originalMemberUUIDs = ref<string[]>([]);
...
const openMembersDrawer = async (role: Role) => {
  ...
  try {
	...
	originalMemberUUIDs.value = [...(detail?.member_uuids ?? [])];
	selectedMemberUUIDs.value = [...originalMemberUUIDs.value];
  } catch (err: any) {
    toast.add({
      title: t("common.error"),
      description: err?.message || t("iam.roles.notifications.loadFailed"),
...
const toggleMemberSelection = (memberUUID: string) => {
	const next = new Set(selectedMemberUUIDs.value);
	if (next.has(memberUUID)) {
		next.delete(memberUUID);
	} else {
		next.add(memberUUID);
	}
	selectedMemberUUIDs.value = Array.from(next);
};
...
const saveMembers = async () => {
  ...
  try {
		const current = new Set(selectedMemberUUIDs.value);
		const previous = new Set(originalMemberUUIDs.value);
    const toAdd = [...current].filter((id) => !previous.has(id));
    const toRemove = [...previous].filter((id) => !current.has(id));

    if (toAdd.length) {
		await iamService.addRoleMembers(activeRole.value.role_uuid, {
			tenant_uuid: tenantUuid,
			member_uuids: toAdd,
      });
    }
    if (toRemove.length) {
		await iamService.removeRoleMembers(activeRole.value.role_uuid, {
			tenant_uuid: tenantUuid,
			member_uuids: toRemove,
      });
    }
        ...
        <div class="p-4 space-y-4">
          ...
          >
            ...
            >
              ...
              <UCheckbox
				:model-value="selectedMemberUUIDs.includes(member.member_uuid)"
				@update:model-value="() => toggleMemberSelection(member.member_uuid)"
              />
            </label>
            <p
          ...
          <p class="text-sm text-gray-500 dark:text-gray-400">
            {{
              t("iam.roles.membersDrawer.selectionSummary", {
				count: selectedMemberUUIDs.length,
              })
            }}
          </p>
