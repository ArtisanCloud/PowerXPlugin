  ...
  <UContainer class="py-10 space-y-6">
    ...
    <UCard>
      ...
      <UTable :rows="subscriptions" :columns="columns" :loading="loading">
        ...
        <template #retry_policy-cell="{ row }">
    ...
    <UModal v-model:open="createOpen">
      <UCard>
        ...
        <form class="space-y-4" @submit.prevent="submitCreate">
          ...
          <UFormField label="目标 URL" required>
            <UInput v-model="createForm.targetUrl" placeholder="https://example.com/webhooks" />
          </UFormField>

          <UFormField label="签名密钥 (可选)" help="用于签名头 X-PowerX-Signature 的密钥，建议使用随机字符串">
            <UInput v-model="createForm.secret" placeholder="留空则沿用默认密钥策略" />
          </UFormField>

          <UFormField label="重试策略 (秒)" help="填写逗号分隔的秒数，例如 60,300,900">
      ...
      <UCard class="flex flex-col h-full">
        ...
        <div class="flex-1 overflow-y-auto">
          <UTable :rows="attempts" :columns="attemptColumns" :loading="attemptLoading">
            ...
            <template #payload_snapshot-cell="{ row }">
...
const attemptColumns = useNormalizedColumns([
  { key: "status", label: "状态" },
