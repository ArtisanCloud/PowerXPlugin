...
const instanceId = 'dev-bridge'
const fullscreenResult = ref('')
...
onMounted(() => {
  setupHostBridgeAdapter({ debug: true, pluginId, instanceId })
})
...
function requestFullscreen(action: 'enter' | 'exit' | 'toggle') {
  const options = { instanceId, reason: 'bridge-dev' }
  const sent = action === 'enter'
    ? requestPowerXHostFullscreen(pluginId, options)
    : action === 'exit'
      ? exitPowerXHostFullscreen(pluginId, options)
      : togglePowerXHostFullscreen(pluginId, options)
  fullscreenResult.value = sent
    ? t('demo.fullscreenSent', { action })
    : t('demo.fullscreenUnavailable')
}
</script>
...
<template>
  <div class="wrap">
    ...
    <div class="ops fullscreen-ops">
      <button @click="requestFullscreen('enter')">{{ t('demo.enterFullscreen') }}</button>
      <button @click="requestFullscreen('exit')">{{ t('demo.exitFullscreen') }}</button>
      <button @click="requestFullscreen('toggle')">{{ t('demo.toggleFullscreen') }}</button>
    </div>
    <p v-if="fullscreenResult" class="result">{{ fullscreenResult }}</p>
  </div>
</template>
...
<i18n lang="json">
{
  "zh": {
    "demo": {
      ...
      "enterFullscreen": "请求全屏",
      "exitFullscreen": "退出全屏",
      "toggleFullscreen": "切换全屏",
      "fullscreenSent": "已发送全屏请求：{action}",
      "fullscreenUnavailable": "当前没有宿主容器可处理全屏请求"
    }
  },
  "en": {
    "demo": {
      ...
      "fullscreenSent": "Fullscreen request sent: {action}",
      "fullscreenUnavailable": "No host container is available for fullscreen requests"
    }
  }
}
</i18n>
...
.fullscreen-ops { padding-top:12px; border-top:1px solid #d4d4d8; }
.result { margin-top:10px; color:#16a34a; font-size:14px; }
