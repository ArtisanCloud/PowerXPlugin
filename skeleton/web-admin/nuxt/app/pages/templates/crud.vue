  ...
  <UContainer class="py-10 space-y-8">
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-2">
        <UInput
          v-model="notificationMemberUUID"
          size="sm"
          class="w-[280px]"
          placeholder="member_uuid（自动识别，失败可手填）"
        />
        <UButton
          size="sm"
          color="info"
...
const notificationMemberUUID = ref("")
...
const sendTestNotification = async () => {
  const inputMemberUUID = String(notificationMemberUUID.value || "").trim()
  if (!inputMemberUUID) {
    const guessed = guessMemberUUID()
    if (guessed) {
      notificationMemberUUID.value = guessed
    }
  }

  const memberUUID = String(notificationMemberUUID.value || "").trim()
  if (!memberUUID) {
    showToast({
      title: "发送失败",
    ...
    await invokeCapability(
      ...
      "Send",
      {
        ...
        body: {
          ...
          title: "Plugin 测试通知",
          content: "这是一条来自模板页按钮的测试消息",
          type: "info",
          category: "system",
          is_important: false,
          metadata: {
            source: "plugin-template-crud",
          },
        },
      },
      {
...
onMounted(async () => {
  ...
  if (!notificationMemberUUID.value) {
    notificationMemberUUID.value = guessMemberUUID()
  }
