const resolvePluginId = () => {
  const processEnvPluginId =
    typeof process !== "undefined"
      ? (process.env.POWERX_PLUGIN_ID ||
          process.env.NUXT_PUBLIC_POWERX_PLUGIN_ID ||
          "")
      : "";
  const viteEnvPluginId = (import.meta as any)?.env?.NUXT_PUBLIC_POWERX_PLUGIN_ID || "";
  return String(processEnvPluginId || viteEnvPluginId || "com.powerx.plugins.base").trim();
};

export const PLUGIN_ID = resolvePluginId();
