...
- [X] T005 初始化 `framework/backend/go/go.mod` 并声明模块 `github.com/ArtisanCloud/PowerXPlugin/framework/backend/go`
...
**Independent Test**: `go run ./skeleton/backend/go-gin/cmd/plugin` 返回 `GET /api/v1/ping`=200；在独立项目引用 `github.com/ArtisanCloud/PowerXPlugin/framework/backend/go` 与 `@artisan-cloud/plugin-framework-admin` 可成功构建/启动。
...
- [X] T017 [US1] 创建 `skeleton/backend/go-gin/go.mod`，引用 `github.com/ArtisanCloud/PowerXPlugin/framework/backend/go`
