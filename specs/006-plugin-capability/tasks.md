...
- [X] T010 Define Capabilities Manager interfaces in `skeleton/backend/go-gin/internal/capabilities/manager.go` (ListCapabilities/ExportProtocols/RegisterWithHost) and wire dependency injection.
...
- [X] T014 Wire failure handling + rollback hooks in plugin installer (`skeleton/backend/go-gin/cmd/server/runtime/install.go`) when `capability_registry` sync返回失败。
- [X] T015 [US5] Enforce default sync/async contract：在 `scripts/capabilities/export.ts` 与 `skeleton/backend/go-gin/internal/capabilities/manager.go` 校验 `async` 标记，默认同步执行，async 能力需声明回调/SSE + 状态查询字段。
...
- [X] T101 [US1] 更新 web-admin 表单页面 `skeleton/web-admin/nuxt/app/pages/capabilities/RegisterForm.vue`，支持字段模板、多语言提示、草稿恢复。
- [X] T102 [P][US1] 扩展 CLI `scripts/capabilities/registry-cli.mjs`，新增 `capabilities new` 与 Schema stub 生成。
- [X] T103 [US1] 实现后台 Handler `skeleton/backend/go-gin/internal/transport/http/admin/capability/register_handler.go`，接入服务层校验/审计。
- [X] T104 [US1] 在 `skeleton/backend/go-gin/internal/services/capability/register_service.go` 实现命名冲突检查、Schema 校验、ID 生成、审计写入。
- [X] T105 [US1] 添加 Go 单元测试 `skeleton/backend/go-gin/internal/services/capability/register_service_test.go` 覆盖冲突/草稿场景。
...
- [X] T201 [US2] 更新服务 `skeleton/backend/go-gin/internal/services/capability_review/workflow_service.go`，根据敏感度生成任务、SLA、升级策略。
- [X] T202 [P][US2] 扩展 `skeleton/backend/go-gin/internal/transport/http/admin/capability/review_handler.go` 支持评论、附件、整改再提交。
- [X] T203 [US2] 实现复核/升级事件推送（Notification Center）于 `skeleton/backend/go-gin/internal/observability/capability/review_events.go`。
- [X] T204 [US2] 编写审核流程集成自测 `skeleton/backend/go-gin/tests/integration/capability_review_flow_test.go`（需位于 skeleton 模块下以访问 internal 包）。
...
- [X] T301 [US3] 提供多协议通道配置 UI `skeleton/web-admin/nuxt/app/pages/capabilities/ExposureForm.vue`（REST/GraphQL/gRPC/Webhook/Workflow/Agent/SDK），支持限流/额度。
- [X] T302 [US3] 在 `skeleton/backend/go-gin/internal/services/capability/exposure_service.go` 同步渠道配置、生成 docs bundle、SDK bundle。
- [X] T303 [P][US3] 实现租户授权 API `skeleton/backend/go-gin/internal/transport/http/admin/capability/quota_handler.go`，支持额度调整与审计。
...
- [X] T306 [US6] 在 `skeleton/web-admin/nuxt/app/pages/capabilities/RegisterForm.vue` 与 `useCapabilityLab` 中新增“本地调试”适配器：默认直接调用插件 REST/gRPC/Workflow 端口，支持在 UI 中展示/切换模式，且无需依赖 `/api/v1/integration/capabilities/invoke` 或租户授权。
...
- [X] T401 [US4] 实现变更申请入口 `skeleton/web-admin/nuxt/app/pages/capabilities/Lifecycle.vue`，允许上传 diff、配置灰度窗口。
- [X] T402 [US4] 服务层 `skeleton/backend/go-gin/internal/services/capability_lifecycle/plan_service.go` 生成差异报告、触发通知、控制双版本并行。
- [X] T403 [US4] 实现通知编排 `skeleton/backend/go-gin/internal/observability/capability/lifecycle_notifier.go`，保证 100% 覆盖与失败重试。
...
- [X] T502 [US5] 实现 Capabilities Manager 与宿主的 REST 客户端 `skeleton/backend/go-gin/internal/integrations/powerx/capability_client.go`（调用 `/internal/plugins/capabilities/**`）。
- [X] T503 [US5] 在插件安装流程 `skeleton/backend/go-gin/cmd/plugin/runtime/install.go` 调用 Cap Manager → PowerX，同步失败立即回滚。
- [X] T504 [US5] 编写 Workflow/Agent E2E 验证脚本 `skeleton/backend/go-gin/tests/integration/capability_catalog_sync_test.go`，覆盖协议资产导出与宿主注册。
- [X] T505 [US5] 增加 async 能力端到端校验脚本 `skeleton/backend/go-gin/tests/integration/capability_async_mode_test.go`，验证默认同步与 async 回调/SSE 行为。
- [X] T506 [US5] 实现 MCP REGISTER/ACK/INVOKE API：在 `skeleton/backend/go-gin/internal/transport/http/admin/runtime_ops/sessions_handler.go` 与 `services/admin/runtime_ops/mcp_session.go` 完成会话注册、心跳、关闭处理，并把请求委托给 `integration.SessionAdapter`。
...
- [X] T508 [US5] 补充集成测试 `skeleton/backend/go-gin/tests/integration/mcp_agent_mode_test.go`，验证同一能力可被 REST Agent 与 MCP Client 同时调度，覆盖 REGISTER→INVOKE→SSE 推送。
...
- [X] T902 [US5] Service 支撑：为 `TemplateService` 添加 `BatchClone` 与 `Validate` 方法（含 repository/transaction 与规则执行器），新增测试 `skeleton/backend/go-gin/internal/services/admin/templates/template_service_batch_clone_test.go`、`template_service_validate_test.go` 覆盖成功/失败路径。
