...
- [x] T005 Implement `Event` meta builder helper (tenant_uuid/request_id/source_plugin/trace_id/occurred_at/payload_version) in `skeleton/backend/internal/domain/event/meta.go`
- [x] T006 Implement core event types (`Event`, `Topic`, `Subscription` DTOs) in `skeleton/backend/internal/domain/event/models.go`
- [x] T007 Implement an emitter interface and factory (local vs taskbus) in `skeleton/backend/internal/services/event_bridge/emitter.go`
- [x] T008 Implement a local emitter (log/in-memory) with fallback semantics in `skeleton/backend/internal/services/event_bridge/local_emitter.go`
...
- [x] T016 [US2] Implement taskbus emitter adapter skeleton (interface only; no external deps) in `skeleton/backend/internal/services/event_bridge/taskbus_emitter.go`
- [x] T017 [US2] Implement fallback logic: taskbus failure → local emitter in `skeleton/backend/internal/services/event_bridge/emitter.go`
...
- [x] T033 [P] [US2] Implement an in-process TaskBus stub for integration tests (publish → dispatch → consumer) in `skeleton/backend/internal/services/event_bridge/taskbus_stub.go`
...
- [x] T023 [US3] Implement a sample consumer interface + local dispatcher (in-memory) in `skeleton/backend/internal/services/event_bridge/consumer.go`
- [x] T024 [US3] Implement idempotency filter using `topic + tenant_uuid + trace_id` in `skeleton/backend/internal/services/event_bridge/idempotency.go`
...
- [x] T037 [US3] Implement dual-write behavior (emit to TaskBus + local) with clear error semantics in `skeleton/backend/internal/services/event_bridge/emitter.go`
