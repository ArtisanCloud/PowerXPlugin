...
### User Story 3 - Task 驱动与前端消费解耦 (Priority: P3)

任务执行由后端 Task 机制统一驱动，前端只负责消费事件，不通过页面轮询触发任务执行。

**Why this priority**: 避免把执行链路耦合到页面轮询，确保宿主/standalone 行为一致。

**Independent Test**: 触发任务执行时，无需页面轮询也能看到事件驱动的状态推进。

**Acceptance Scenarios**:

1. **Given** 任务由后端触发执行，**When** 任务状态变化，**Then** 前端可通过统一事件订阅收到状态更新，而非通过轮询触发执行。
...
- **SC-003**: 任务执行链路不依赖页面轮询触发，前端仅消费事件更新。
