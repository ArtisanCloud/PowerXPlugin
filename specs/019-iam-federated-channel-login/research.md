...
## Decision 8: 交付顺序切换为“飞书优先，钉钉延后联调”

- **Decision**: 当前迭代优先完成飞书渠道闭环验收；钉钉保持接口兼容与代码路径可用，但不作为发布阻塞项。  
- **Rationale**: 钉钉当前账号体系触发服务商入驻/费用门槛，导致联调时间不可控；飞书侧可直接进入自建应用联调，交付风险更低。  
- **Alternatives considered**:
  - 继续钉钉优先：理论模型更接近企微，但受账号与商务流程阻断，无法保证迭代节奏。
  - 同时暂停钉钉与飞书：风险最低但无法持续推进多渠道目标。
