...
func RefreshToolToken(ctx context.Context, cfg *config.Config) (string, string, error) {
	...
	endpoints := buildRefreshEndpoints(base)
	if len(endpoints) == 0 {
...
func buildRefreshEndpoints(base string) []string {
	trimmedBase := strings.TrimRight(strings.TrimSpace(base), "/")
	if trimmedBase == "" {
		return nil
	}
	baseWithoutAPIV1 := strings.TrimSuffix(trimmedBase, "/api/v1")
	if baseWithoutAPIV1 == "" {
		baseWithoutAPIV1 = trimmedBase
	}
	candidates := []string{
		baseWithoutAPIV1 + "/api/v1/admin/user/auth/refresh",
		trimmedBase + "/admin/user/auth/refresh",
		baseWithoutAPIV1 + "/admin/user/auth/refresh",
	}
