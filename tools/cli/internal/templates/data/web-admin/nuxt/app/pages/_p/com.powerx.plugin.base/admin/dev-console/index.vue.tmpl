<template>
  <div class="space-y-6">
    <header class="space-y-2">
      ...
      <div>
        ...
        <p class="text-sm text-gray-600 dark:text-gray-300">
          管理 Dev Console 的默认参数、导出策略和安全操作。
        </p>
      </div>
    </header>
      ...
      >
        <template #actions>
          <AuditExportDialog
            ...
            :pending="auditStore.exporting"
            @export="exportAuditEvents"
          />
        </template>
      </AuditHistoryTable>
    </section>
    ...
    <section v-else class="space-y-6">
      <UAlert v-if="runsError" color="red" variant="soft" :title="runsError" />
      ...
      >
        <template #actions>
          <UButton size="xs" variant="soft" color="primary" @click="loadJobRuns"
            >刷新列表</UButton
          >
        </template>
      </JobRunsTable>
      ...
      <UCard>
        ...
        <form class="grid gap-4 md:grid-cols-3" @submit.prevent="submitSafeOp">
          ...
          <UFormField label="作用域类型">
            <USelect
              v-model="safeOpForm.scope_type"
              :options="safeOpScopeTypes"
            />
          </UFormField>
          <UFormField label="作用域标识">
            <UInput v-model="safeOpForm.scope_ref" placeholder="tenant-1" />
          </UFormField>
          <UFormField label="目标 ID" class="md:col-span-3">
            <UInput v-model="safeOpForm.target_id" placeholder="hook-evt-123" />
          </UFormField>
          <UFormField label="备注" class="md:col-span-3">
            <UTextarea v-model="safeOpForm.reason" placeholder="本次操作原因" />
          </UFormField>
          <UFormField label="Dry Run" class="md:col-span-3">
            <USwitch v-model="safeOpForm.dry_run" />
          </UFormField>
          <UFormField label="租户 ID" class="md:col-span-3">
            <UInput v-model="safeOpForm.tenant_uuid" placeholder="tenant-1" />
          </UFormField>
          <div class="md:col-span-3 flex justify-end gap-3">
            <UButton type="submit" color="primary" :loading="safeOpsLoading"
              >提交操作</UButton
            >
          </div>
        </form>
...
import type {
  ...
  JobRunFilters,
  SafeOpPayload,
} from "~/stores/dev-console/troubleshoot";
...
watch(
  () => jobFilters.value.tenant_uuid,
  (value) => {
    const cleaned = value?.trim() ?? "";
    safeOpForm.tenant_uuid = cleaned;
...
watch(
  ...
  (value) => {
    if (value === "tenant" && jobFilters.value.tenant_uuid) {
      safeOpForm.scope_ref = jobFilters.value.tenant_uuid;
    }
  }
);
...
async function loadJobRuns(cursor?: string) {
  ...
  if (payload.tenant_uuid) payload.tenant_uuid = payload.tenant_uuid.trim();
...
async function handleRetry(run: JobRun) {
  retrying.value = true;
  try {
    await troubleshootStore.retryRun(run.id, {
      tenant_uuid: jobFilters.value.tenant_uuid,
    });
...
async function refreshSummary() {
  const tenant = jobFilters.value.tenant_uuid?.trim();
  try {
    await troubleshootStore.fetchSummary({ tenant_uuid: tenant || undefined });
  } catch (error: any) {
    toast.add({
      title: "加载看板失败",
      description: error?.message ?? "请稍后再试",
...
async function submitSafeOp() {
  ...
  const payload: SafeOpPayload = {
    ...
    tenant_uuid: safeOpForm.tenant_uuid?.trim() || undefined,
